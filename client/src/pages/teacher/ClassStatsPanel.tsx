...
function SimpleBarChart({ data, color = 'var(--color-accent)' }: { data: { label: string; value: number }[]; color?: string }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 2, height: 120, overflowX: 'auto', paddingBottom: 20, position: 'relative' }}>
      {data.map((d, i) => (
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', maxWidth: W, overflow: 'visible' }}>
      {[0, 0.25, 0.5, 0.75, 1].map((v) => (
        <g key={v}>
          <line x1={PAD.left} y1={toSvgY(v)} x2={W - PAD.right} y2={toSvgY(v)} stroke="var(--color-border)" strokeWidth={1} />
